...
const AdminUsers: React.FC = () => {
    ...
    const handleDeleteUser = async () => {
        ...
        setIsDeleting(true);
        console.log(`[AdminUsers] Deleting user ${deletingUser.email} for reason: ${deletionReason}`);
                ...
                if (RESEND_API_KEY) {
                    await fetch('https://api.resend.com/emails', {
                        ...
                        body: JSON.stringify({
                            ...
                            subject: 'Account Deleted - Plaiz Studio',
                            html: `
                                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                                    <h2 style="color: #ff4d4d;">Account Deleted</h2>
                                    <p>Hello ${deletingUser.name || 'User'},</p>
                                    <p>Your account at Plaiz Studio has been deleted by an administrator.</p>
                                    <div style="background: #f5f5f5; padding: 15px; border-radius: 8px; margin: 20px 0;">
                                        <strong>Reason for deletion:</strong><br/>
                                        ${deletionReason}
                                    </div>
                                    <p>If you have any questions, please contact our support team.</p>
                                    <p>Best regards,<br/>Plaiz Studio Team</p>
                                </div>
                            `
                        })
                    });
    ...
    const filteredInvites = inviteCodes.filter(c => {
        const query = searchQuery.toLowerCase();
        if (!query) return true;
        return (
            ...
            <div className="space-y-8 animate-fade-in pb-12 relative">
                            ...
                            <input
                                ...
                                placeholder={
                                    activeTab === 'users' ? "Search by name or email..." :
                                        activeTab === 'invites' ? "Search codes or email..." :
                ...
                {deletingUser && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
                        <div className="w-full max-w-md bg-[#0f172a] border border-white/10 rounded-3xl p-8 relative shadow-2xl">
                            <button
                                onClick={() => { setDeletingUser(null); setDeletionReason(''); }}
                                className="absolute top-4 right-4 p-2 text-white/20 hover:text-white transition-colors"
                            >
                                <X size={20} />
                            </button>
                            ...
                            <p className="text-white/40 text-sm font-medium mb-6">You are deleting <strong>{deletingUser.name || deletingUser.email}</strong>. This action is permanent.</p>

                            <div className="space-y-4">
                                <div>
                                    <label className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2">Reason for Deletion</label>
                                    <textarea
                                        value={deletionReason}
                                        onChange={(e) => setDeletionReason(e.target.value)}
                                        placeholder="State the reason for deletion..."
                                        className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white text-sm focus:border-plaiz-coral focus:ring-1 focus:ring-plaiz-coral outline-none transition-colors min-h-[120px] resize-none"
                                        required
                                    />
                                    <p className="text-[10px] text-white/20 font-medium mt-2">This reason will be sent to the user via email.</p>
                                </div>

                                <button
                                    onClick={handleDeleteUser}
                                    disabled={isDeleting || !deletionReason.trim()}
                                    className="w-full py-4 bg-plaiz-coral text-white font-bold rounded-xl transition-all hover:brightness-110 active:scale-95 disabled:opacity-50 disabled:active:scale-100 flex items-center justify-center gap-2"
                                >
                                ...
                                <button
                                    onClick={() => { setDeletingUser(null); setDeletionReason(''); }}
                                    className="w-full py-3 bg-white/5 text-white/60 font-bold rounded-xl transition-colors hover:text-white"
                                >
                                    Cancel
                                </button>
                            </div>
